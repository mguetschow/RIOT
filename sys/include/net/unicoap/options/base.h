...
/**
 * @file
 * @brief  Option structures and base accessors
...
/**
 * @brief Gets the next query option matching a given null-terminated name string,
 ...
 * @param[in,out] iterator Option iterator
 * @param[out] number Option number
 * @param[in] name Name of UTF-8 query parameter to find, must be null-terminated
 * @param[out] value Pointer to a UTF-8 string variable
 *
 * @returns Positive size of option value or Negative integer on error
 * @retval `-EBADOPT` Option is corrupted
 ...
 */
static inline
ssize_t unicoap_options_get_next_query_by_name_string(unicoap_options_iterator_t* iterator,
                                               unicoap_option_number_t number, const char* name,
                                               const char** value)
{
    return unicoap_options_get_next_query_by_name(iterator, number, name, strlen(name), value);
}
